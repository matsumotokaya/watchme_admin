...
### 🎯 設計思想
**API呼び出しは本番環境マイクロサービスを参照**
- ローカル・本番環境に関係なく、管理画面は本番APIのみを呼び出し
- 各機能のタイトル下に表示されているURLが実際の参照先
...
#### 機能概要
- **目的**: 指定したデバイスIDと日付の音声データを手動で文字起こし
...
#### 🔄 2025-07-18 重要アップデート: audio_filesテーブル統合

##### 変更前の問題点
- vibe_whisperテーブル（処理済みデータ）のみを参照
- 新しい音声ファイルを永遠に検出できない
- 全48スロットを盲目的に試行（無駄な404エラー）
- file_pathの情報を活用していない

##### 変更後の改善点
- **audio_filesテーブルを参照**: 未処理ファイル（`whisper_status='pending'`）を検出
- **file_pathの活用**: 実際に存在するファイルのみを効率的に処理
- **無駄な処理の削減**: 404エラーを回避し、処理時間を大幅短縮
- **正確な処理対象の把握**: どのファイルが処理対象かを明確に表示

#### 処理フロー（改善版）

```python
# 1. audio_filesテーブルから未処理ファイルを取得
pending_files = SELECT * FROM audio_files 
                WHERE device_id = ? 
                AND whisper_status = 'pending'
                AND recorded_at >= (NOW() - INTERVAL '24 hours')

# 2. file_pathリストをWhisper APIに送信
{
    "device_id": "xxx",
    "date": "2025-07-18",
    "model": "base",
    "file_paths": [
        "files/xxx/2025-07-18/12-00/audio.wav",
        "files/xxx/2025-07-18/14-00/audio.wav",
        ...
    ]
}
