...
### 🎯 設計思想
**最終的にはすべてのAPI呼び出しが本番環境マイクロサービスを参照する設計**
- ローカル・本番環境に関係なく、管理画面は本番APIのみを呼び出し
- 各機能のタイトル下に表示されているURLが実際の参照先
- ハイブリッド状態は開発途中の一時的なもの

### 📋 移行手順
1. **現在**: 心理グラフ機能の本番API移行完了
2. **次回**: 行動グラフ機能の本番API移行  
3. **将来**: 感情グラフ機能の本番API移行
4. **最終**: 全機能の本番マイクロサービス化完了
...
- **🔄 プロキシアーキテクチャ導入**: 管理画面バックエンドに3つのプロキシエンドポイントを追加
